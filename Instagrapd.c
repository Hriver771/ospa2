...
//
    ...
    sprintf(out_file_name, "out%s.c", sid) ;
    ...
    for(int i=1; i<=10; i++) {
        send_message(thread_sock_fd, "@") ;
        sprintf(test_input, "%s%d.in", file_dir, i) ;
        printf("%s", test_input) ;
        send_file(thread_sock_fd, test_input) ;
	printf("%s\n", test_input) ;
    }
...
main(int argc, char const *argv[]) 
{
    ...
    file_dir = (char *)malloc(sizeof(char)*strlen(argv[optind])) ;
    strcpy(file_dir, argv[optind]) ; 
